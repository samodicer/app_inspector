...
const actions = {
    ...
    async fetchFiles({commit}){
        axios.get('http://127.0.0.1:8000/get-files/').then((response)=>{
            commit('setFiles', response.data);
            console.log(response);
        }).catch((err)=>{console.log(err)});
    },
    async analyzeFile({commit},id){
        axios.get('http://127.0.0.1:8000/get-file-data/'+id).then((response)=>{
...
const mutations = {
    ...
    setAnalyzedData: (state, analyzed_data) => (state.analyzed_data = analyzed_data) 
};
